...
accordionRouter.route('/:accordionId')
.all((req, res, next) => {
    res.statusCode = 200;
    res.setHeader('Content-Type', 'text/plain');
    next();
...
.get((req, res) => {
    res.end('Will send all accordion data to you');
})
.post((req, res) => {
    res.end(`Will add the accordion: ${req.body.name} with description: ${req.body.description}`)
})
.put((req, res) => {
    res.statusCode = 403;
    res.end('PUT operation not supported on /accordion/${accordionId');
})
.delete((req, res) => {
    res.end(`Deleting accordion.`)
})
